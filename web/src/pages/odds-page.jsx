...
function CalibrationChart({ bands, summary }) {
  ...
  const height = isMobile ? 300 : 410
  const plot = isMobile
    ? { left: 42, right: 12, top: 22, bottom: 52 }
    : { left: 58, right: 28, top: 28, bottom: 62 }
  ...
  const ticks = []
  for (let tick = domainMin; tick <= domainMax; tick += isMobile ? 20 : 10) ticks.push(tick)
  ...
  return (
    <Card className="h-full">
      ...
      <CardContent>
        ...
        <div className="overflow-hidden md:overflow-x-auto" role="region" aria-label="Calibration chart" tabIndex="0">
          <svg viewBox={`0 0 ${width} ${height}`} className="w-full md:min-w-[680px]" role="img" aria-labelledby="calibration-chart-title calibration-chart-desc">
            ...
            {ticks.map((tick) => <g key={tick}><line x1={plot.left} x2={width - plot.right} y1={scaleY(tick)} y2={scaleY(tick)} stroke="var(--border)" /><line x1={scaleX(tick)} x2={scaleX(tick)} y1={plot.top} y2={height - plot.bottom} stroke="var(--border)" strokeDasharray="2 5" /><text x={plot.left - 8} y={scaleY(tick) + 4} textAnchor="end" fontSize={isMobile ? "9" : "11"} fill="var(--muted-foreground)">{tick}%</text><text x={scaleX(tick)} y={height - plot.bottom + (isMobile ? 17 : 22)} textAnchor="middle" fontSize={isMobile ? "9" : "11"} fill="var(--muted-foreground)">{tick}%</text></g>)}
            <line x1={scaleX(domainMin)} y1={scaleY(domainMin)} x2={scaleX(domainMax)} y2={scaleY(domainMax)} stroke="var(--chart-2)" strokeWidth="1.5" strokeDasharray="5 5" />
            {chartBands.map((row) => {
              const [low, high] = row.interval
              const cx = scaleX(row.avg_implied_pct)
              const cy = scaleY(row.win_rate_pct)
              return <g key={row.band}>
                <line x1={cx} x2={cx} y1={scaleY(high)} y2={scaleY(low)} stroke="var(--chart-1)" strokeWidth="2" opacity=".65" />
                <line x1={cx - 5} x2={cx + 5} y1={scaleY(high)} y2={scaleY(high)} stroke="var(--chart-1)" strokeWidth="2" opacity=".65" />
                <line x1={cx - 5} x2={cx + 5} y1={scaleY(low)} y2={scaleY(low)} stroke="var(--chart-1)" strokeWidth="2" opacity=".65" />
                <circle cx={cx} cy={cy} r="7" fill="var(--chart-1)" stroke="var(--background)" strokeWidth="2"><title>{`${row.band}: ${formatPercent(row.win_rate_pct)} actual vs ${formatPercent(row.avg_implied_pct)} average price; 95% interval ${formatPercent(low)} to ${formatPercent(high)}; n=${row.games}`}</title></circle>
                <text x={Math.min(width - plot.right - 4, cx + (isMobile ? 7 : 10))} y={Math.max(plot.top + 12, cy - (isMobile ? 7 : 10))} fontSize={isMobile ? "9" : "11"} fontWeight="600" fill="var(--foreground)">{row.band.replace("%", "")}</text>
              </g>
            })}
            <text x={plot.left + plotWidth / 2} y={height - (isMobile ? 7 : 12)} textAnchor="middle" fontSize={isMobile ? "9" : "12"} fontWeight="600" fill="var(--muted-foreground)">Average pre-match market price</text>
            <text transform={`rotate(-90 ${isMobile ? 11 : 14} ${plot.top + plotHeight / 2})`} x={isMobile ? "11" : "14"} y={plot.top + plotHeight / 2} textAnchor="middle" fontSize={isMobile ? "9" : "12"} fontWeight="600" fill="var(--muted-foreground)">Observed favorite win rate</text>
          </svg>
        </div>
...
function OutcomeComparison({ games }) {
  ...
  return (
    <Card className="h-full">
      ...
      <CardContent className="grid flex-1 content-evenly gap-8">
        {["Market role", "Venue"].map((partition) => <div key={partition}><p className="mb-3 text-xs font-medium tracking-wide text-muted-foreground uppercase">{partition}</p><div className="space-y-4">{groups.filter((row) => row.partition === partition).map((row) => <div key={row.key}>
          ...
          <div className="relative h-2 overflow-visible rounded-full bg-muted"><span className="absolute inset-y-0 left-0 rounded-full bg-blue-600 dark:bg-blue-500" style={{ width: `${Math.max(0, Math.min(100, row.actual || 0))}%` }} /><span className="absolute -top-1 h-4 w-0.5 bg-zinc-800 dark:bg-zinc-200" style={{ left: `${Math.max(0, Math.min(100, row.price || 0))}%` }} /></div>
        </div>)}</div></div>)}
      </CardContent>
    </Card>
  )
  ...
  return (
    <div className="space-y-6">
      ...
      {!loading && !error && analysis && hasData && <>
        <Card className="border-blue-200 bg-blue-50/50 ring-blue-200 dark:border-blue-900 dark:bg-blue-950/25 dark:ring-blue-900">
          <CardContent className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
            <div><p className="text-xs font-medium tracking-wide text-blue-700 uppercase dark:text-blue-300">Overall finding</p><h2 className="mt-1 text-xl font-semibold tracking-tight">{conclusion}</h2><p className="mt-1 text-sm text-muted-foreground">{formatPercent(summary.favorite_win_rate_pct)} actual versus {formatPercent(summary.avg_favorite_implied_pct)} average price · {formatDelta(delta)} · n={formatNumber(summary.favorite_games)}</p></div>
            <Scale className="size-8 shrink-0 text-blue-700 dark:text-blue-300" />
          </CardContent>
        </Card>

        <BalancedCardGrid className="gap-3 sm:grid-cols-3">
